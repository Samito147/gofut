...
export default async function handler(req, res) {
  ...
  // 🪙 Cria automaticamente a carteira para o novo usuário
  ...
  if (userFetchError || !userRecord) {
    console.error('Erro ao obter usuário recém-criado:', userFetchError)
    return res.status(500).json({ error: 'Usuário criado no Auth, mas não encontrado na tabela users.' })
  }
  ...
  if (walletError) {
    ...
    return res.status(500).json({ error: 'Usuário criado, mas falha ao criar carteira GP.' })
  }
